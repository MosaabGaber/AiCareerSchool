...
export function HomeRealEstate({
  ...
  // Listen for the navbar JOIN NOW button event (dispatched by FloatingNav when on /v2)
  useEffect(() => {
    const handler = () => setIsPaymentOpen(true);
    window.addEventListener('open-v2-payment', handler);
    return () => window.removeEventListener('open-v2-payment', handler);
  }, []);
  ...
  return (
    <div className="bg-white min-h-screen text-gray-900">
      ...
      <main className="mt-6">
        ...
        <section className="py-8 flex flex-col items-center justify-center text-center px-4 relative z-10">
          ...
          <div className="mt-2 flex flex-col items-center gap-3">
            <span className="text-gray-600 font-medium text-sm" dir="rtl">طرق الدفع المتاحة</span>
        ...
        <section className="py-12 px-6 container mx-auto relative z-10 max-w-6xl">
          <div className="text-center mb-10">
            <h2 className="text-3xl md:text-4xl font-outfit font-bold text-gray-900 tracking-tight" dir="rtl">
              نتائج من سوق العقارات
            </h2>
          </div>
      ...
      <footer className="border-t border-gray-200 py-10 mt-12 relative z-10 bg-white">
        <div className="container mx-auto px-6 flex flex-col items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-outfit font-bold text-gray-600">AI Career School &copy; {new Date().getFullYear()}</span>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <button
              ...
              className="ml-4 px-5 py-1.5 rounded-full border border-gray-300 bg-white text-gray-900 hover:bg-gray-50 shadow-sm transition-all duration-300 text-sm font-medium tracking-wide flex items-center justify-center"
            >
              Prompts
            </button>
            ...
            <button
              ...
              className="ml-2 px-5 py-1.5 rounded-full border border-gray-300 bg-white text-gray-900 hover:bg-gray-50 shadow-sm transition-all duration-300 text-sm font-medium tracking-wide flex items-center justify-center cursor-pointer"
            >
              Checkout
            </button>
          </div>
          ...
          <div className="flex items-center text-sm text-gray-600 gap-6">
            ...
            <button onClick={() => setIsPrivacyOpen(true)} className="hover:text-gray-900 transition-colors cursor-pointer">Privacy</button>
            <button onClick={() => setIsTermsOpen(true)} className="hover:text-gray-900 transition-colors cursor-pointer">Terms</button>
            <button onClick={() => setIsRefundOpen(true)} className="hover:text-gray-900 transition-colors cursor-pointer">Refund Policy</button>
            <Link href="/contact" className="hover:text-gray-900 transition-colors text-gray-600">Contact</Link>
          </div>
        </div>
      </footer>
      ...
      <a
        ...
        className="fixed bottom-6 right-6 z-[9999] flex items-center justify-center rounded-full shadow-[0_4px_20px_rgba(0,148,77,0.3)] hover:shadow-[0_4px_28px_rgba(0,148,77,0.5)] hover:scale-110 active:scale-95 transition-all duration-200"
        style={{ width: '56px', height: '56px', background: '#00944d' }}
      >
      ...
      <V2PaymentModal isOpen={isPaymentOpen} onClose={() => setIsPaymentOpen(false)} />
    </div>
  );
}
