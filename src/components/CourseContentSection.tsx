  ...
  return (
    <section className="py-16 px-4 md:px-8 container mx-auto max-w-7xl relative z-10">
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        {/* Left Column - Video */}
        <div className="w-full lg:w-1/2 lg:sticky lg:top-24 z-20">
        ...
        <div className="w-full lg:w-1/2">
          <div className="bg-white/5 border border-white/10 backdrop-blur-md rounded-2xl p-4 md:p-6 lg:max-h-[600px] lg:overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-white/5 [&::-webkit-scrollbar-thumb]:bg-neongreen/50 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-neongreen/80 transition-colors">
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex h-8 w-8 rounded-full bg-neongreen/10 text-neongreen items-center justify-center font-bold text-sm">
                        {chapterIdx + 1}
                      </div>
                      <h3 className="text-neongreen font-bold text-lg md:text-xl text-right">
                        {chapter.chapter}
                      </h3>
                    </div>
                    <div className="text-gray-400 shrink-0">
                      {openChapter === chapterIdx ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                    </div>
                  </button>
                  ...
                  >
                    <div className="p-3 pt-0 pb-4 flex flex-col gap-2" dir="ltr">
                      {chapter.lessons.map((lesson, lessonIdx) => {
                        ...
                        return (
                          ...
                          >
                            <div className="p-3 flex items-start sm:items-center justify-between gap-4">
                              <div className="flex items-center gap-3 flex-1">
                                <PlayCircle size={16} className="text-neongreen shrink-0 mt-0.5 sm:mt-0" />
                                <span className="text-white text-sm md:text-base font-medium">{lesson.title}</span>
                              </div>
                              <span className="text-gray-500 text-xs md:text-sm shrink-0 font-mono mt-0.5 sm:mt-0">{lesson.duration}</span>
                            </div>
                            
                            <div 
                              className={`transition-all duration-300 ease-in-out bg-black/30 overflow-hidden ${
                                isLessonOpen ? 'max-h-40 opacity-100 p-3' : 'max-h-0 opacity-0 px-3 py-0'
                              }`}
                              dir="rtl"
                            >
